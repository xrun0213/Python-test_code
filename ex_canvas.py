...
class App:
    ...
    def init_widgets(self):
        ...
        # 初始化小球的动画帧
        for i in range(8):
            self.cv.bms.append(ImageTk.PhotoImage(file='/Users/KirkShu/7.jpg'))
        # 绘制小球
        self.ball = self.cv.create_image(self.curx, self.cury, image=self.cv.bms[self.ball_index])

        # self.cv.bms = ImageTk.PhotoImage(file="/Users/KirkShu/dot.png")
        # self.ball = self.cv.create_image(self.curx, self.cury, image=self.cv.bms)
